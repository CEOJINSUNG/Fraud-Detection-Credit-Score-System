...
### 개발 방법
    1. Kaggle 내 데이터를 AWS S3에 parquet 형식으로 50개씩 끊어서 업로드
    2. 사용자의 연결된 계좌의 거래 내역을 기반으로 사기 여부 결정 -> 정상군, 위험군으로 분류
    3. 들어온 사용자의 활동 정보를 토대로 개인별, 집단별 구매 패턴 예측 모델 생성
    4. 비정상적 거래가 발생하는 계좌에 대한 이상을 탐지 후 조치
    5. 거래 정지 및 계좌 정지
    6. 비정상적 거래에 대한 케이스 분류 후 패턴 학습 및 생성
